...
public ResponseEntity<Postagens> getById(@PathVariable Long id) {
 ...
 public ResponseEntity<Postagens>post (@Valid @RequestBody Postagens postagem) {
   ...
   @PostMapping
	public ResponseEntity<Postagens> post (@Valid RequestBody Postagem){
		return ResponseEntity.status(HttpStatus.CREATED).body(repository.save(Postagem));
	}
   @PutMapping
   public ResponseEntity<Postagens> put (@Valid @RequestBody Postagens postagen) {
	   return ResponseEntity.status(HttpStatus.OK).body(repository.save(postagen));
   }
 @DeleteMapping("/{id}")
